...
$file = __DIR__ . '/../core/class/googletvbe.class.php';
...
if (!preg_match('/class\s+googletvbeCmd\s+extends\s+cmd/', $source)) {
    $problems[] = 'Classe googletvbeCmd absente — impossible de créer un équipement.';
}
...
$copy = sys_get_temp_dir() . '/googletvbe.check.' . getmypid() . '.php';
...
$script = 'require_once ' . var_export($core, true) . ';'
        ...
        . ' foreach (array("googletvbe" => "eqLogic", "googletvbeCmd" => "cmd") as $c => $p) {'
